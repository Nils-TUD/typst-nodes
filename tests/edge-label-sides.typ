...
#cetz.canvas({
  ...
  edge("a.east", "b.west", label: [north], label-pos: (50%, "north"), mark: (end: ">"))

  // horizontal edge — label south (below line)
  edge("c.east", "d.west", label: [south], label-pos: (50%, "south"), stroke: blue, mark: (end: ">"))

  // vertical edge — label east (right of line)
  edge("b.south", "d.north", label: [east], label-pos: (50%, "east"), stroke: red, mark: (end: ">"))

  // vertical edge — label west (left of line)
  edge("a.south", "c.north", label: [west], label-pos: (50%, "west"), stroke: green, mark: (end: ">"))

  // bare ratio label-pos (no side tuple) — defaults to "north"
  edge("a", "d", label: [bare 30%], label-pos: 30%, stroke: orange, mark: (end: ">"))
})
